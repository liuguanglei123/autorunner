...
XMLText* XMLDocument::NewText(const int num)
{
...
XMLText* XMLDocument::NewText(const LONGLONG num)
{
...
XMLText* XMLDocument::NewText(CString string)
{
	wchar_t *wChar = string.GetBuffer(string.GetLength());
	string.ReleaseBuffer();
	// 将得到的wchar* 类型转为 char*类型
	size_t length = wcslen(wChar) + 1;
	size_t converted = 0;
	char *cChar;
	cChar = (char*)malloc(length * sizeof(char));
	wcstombs_s(&converted, cChar, length, wChar, _TRUNCATE);
	return  NewText(cChar);
}
...
XMLError XMLDocument::Parse(CString p, size_t len)
{
	wchar_t *wChar = p.GetBuffer(p.GetLength());
	p.ReleaseBuffer();
	// 将得到的wchar* 类型转为 char*类型
	size_t length = wcslen(wChar) + 1;
	size_t converted = 0;
